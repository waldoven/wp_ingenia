...
define('business_store_THEME_URL','https://www.ceylonthemes.com/product/wordpress-storefront-theme/');
define('business_store_THEME_AUTHOR_URL','http://www.ceylonthemes.com');
define('business_store_THEME_DOC','https://www.ceylonthemes.com/wp-tutorials/');
...
function business_store_general_admin_notice(){
	...
	if($pagenow == 'index.php' || $pagenow == 'themes.php'){
         ...
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
             	<p><a href=%1$s target="_blank"  style="text-decoration: none; margin-left:10px;" class="button button-primary"> %2$s </a>
			 	<a href=%3$s target="_blank"  style="text-decoration: none; margin-left:10px;" class="button">%4$s</a>
			 	<a href="?business_store_notice_dismissed" target="_self"  style="text-decoration: none; margin-left:10px;" class="button">%5$s</a>
			 	<strong>%6$s</strong></p></div>',				
				esc_url(business_store_THEME_DOC),
				esc_html__('Tutorials','business-store'),
				esc_url(business_store_THEME_URL),	
				esc_html__('Pro Features','business-store'),
				esc_html__('Dismiss','business-store'), 
				esc_html__(' 1. Enable | Disable WooCommerce Slider, Navigation and Banner :- Appearance -> Customize -> Theme Options', 'business-store'));				
...
if ( isset( $_GET['business_store_notice_dismissed'] ) ){
	update_option('business_store_notice', false);
}
$business_store_notice = get_option('business_store_notice', true);
if($business_store_notice){
