(function($){
    ...
    OpenShopAdmin = {
        ...
        _activetedPlugin: function(event, args){
                event.preventDefault();
                var $card = jQuery( '.'+args.slug);
                OpenShopAdmin._activePluginHomepage(args.slug,$card.data('init'));
        },
        /**
        ...
        _activePluginHomepage: function($slug,$init){
            ...
            if($init=='hunk-companion/hunk-companion.php' || $init=='one-click-demo-import/one-click-demo-import.php'|| $init=='woocommerce/woocommerce.php'){
                     if(jQuery('button.hunk-companion').hasClass('disabled') && jQuery('button.one-click-demo-import').hasClass('disabled') && jQuery('button.woocommerce').hasClass('disabled')){ 
                            var output = '<a href="'+ openshop.openshopSitesLink +'" aria-label="'+ openshop.openshopSitesLinkTitle +'">' + openshop.openshopSitesLinkTitle +' </a>';
        ...
        _activePlugin: function(event){
                ...
                OpenShopAdmin._activePluginHomepage($slug,$init);
            },
        _bind: function(){               
            $( document ).on('click'                     , '.install-now', OpenShopAdmin._installNow);
            $( document ).on('click'                     , '.activate-now', OpenShopAdmin._activePlugin);
            $( document ).on('wp-plugin-install-error'   , OpenShopAdmin._installError);
            $( document ).on('wp-plugin-installing'      , OpenShopAdmin._pluginInstalling);
            $( document ).on('wp-plugin-install-success' , OpenShopAdmin._activetedPlugin);  
        },
